...
class Task1 extends Util {
   ...
   * 批量上架
   */
  async batchUp(sjIds, user) {
    this.log(`>>>batchUp`)
    await this.page.evaluate(() => {
      $('[data-val="wubanotspread"]').click();
      $('[data-unityinfoid]').remove();
    })
    await this.sleep(500);
    await this.page.evaluate(() => {
      $('button.ui-button.ui-button-small.search-btn').click();
    ...
    await this.sleep(500);
    await this.page.waitForSelector('table.ui-table.sydc-table tbody tr');
    let pages = await this.page.evaluate(() => {
      return ($('#pager [data-page]').length || 0) + 1
    });
    for (let i = pages - 1; i >= 0; i--) {
      ...
      await this.page.waitForSelector('table.ui-table.sydc-table tbody tr');
      //匹配当前页面待上架的ids
      await this.page.evaluate((sjIds) => {
        let ids = [...$('.phase span:contains(编号)').toArray().map(item => {
          let span = $(item).text();
          return span.match(/\d{10,}/)[0]
        })] || [];
        for (let i = 0; i < ids.length; i++) {
          if (sjIds.find(item => item.shopId == ids[i])) {
            alert(ids[i])
            alert($(`[data-unityinfoid="${ids[i]}"] [type=checkbox]`).length)
            $(`[data-unityinfoid="${ids[i]}"] [type=checkbox]`).click();
            // $(`[data-unityinfoid="${ids[i]}"] [type=checkbox]`).prop('checked', true);
          }
        }
      }, sjIds)
      console.log(6666);
