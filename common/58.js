...
function getHouseDetail() {
  ...
  localStorage.setItem('houseDetailValue', window.houseDetailValue);
...
function addHouseInfoData(code, userName) {
  ...
  if (houseDetailData.moneyNum && parseFloat(houseDetailData.moneyNum)) {
    ...
    houseDetailData.passengerFlowArr.map(name => {
      $('[name=params_210] label:contains(' + name + ')').click();
    });
    //租金
...
function updateHouseInfo(key, status, houseUrl, type) {
  console.log('key:' + key);
  ...
  if (type === 1) {
    ...
    //bs
    url = `http://010shangpu.com/api/openchangegeneralizestatus`
    data = {
      transfer_store_id: key
    }
  } else if (type === 4) {
    //998
    url = `http://998pu.com/api/open/changegeneralizestatus`
    data = {
      transfer_store_id: key
    }
  }
